  ...
  <div class="app-container">
    ...
    <section>
      <ul>
        ...
        <li>
          <h4>
            v-bind：指令可以对标签绑定属性和组件绑定一些Props
            后面绑定一个Arguments参数作为指令绑定的属性
          </h4>
          <p :title="':props'">v-bind指令简写用 <b>:props</b>进行简写</p>
          <p v-bind:style="{ color: 'red' }">
            绑定属性值为对象形式：使用v-bind绑定style属性给文本设置红色字体
          </p>
          <p :class="{ textColorBlue: true, fontSizeBase: false }">
            为元素动态绑定calss属性
            <b>{ textColorBlue: true, fontSizeBase: false }</b>true就添加对于类否则不添加
          </p>
        ...
        <li>
          ...
          <button v-on:click.stop="handleKeyDown" v-on:click="count++" @click="handleClick">
            点赞{{ count }}
          </button>
        </li>
        <hr />
        ...
        <li>
          <h4>
            v-bind和v-on后面也可以绑定动态参数语法v-bind:[attr]
            v-on:[evnentName]，对于动态参数的值只能是字符串和null
          </h4>
        ...
        <li>
          ...
          <div @click.capture="handleParentClick">
            parent element
            <div @click="handleChildClick">child elememt</div>
          </div>
          <hr />
          ...
          <div @click="handleParentClick">
            parent element
            <div @click.stop="handleChildClick">child elememt</div>
          </div>
          <hr />
          ...
          <div @click.self="handleParentClick">
            parent element
            <div>child elememt</div>
          </div>
          <hr />

          <h5>once：只会执行一次</h5>
          <el-button @click.once="handleParentClick"> 只会执行一次 </el-button>
          <hr />
